...
int status;
	...
	for(unsigned j = 0; j < N; ++j) {
	  for(unsigned k = 0; k < N; ++k){
	    //input_a[j*N+k] = int(rand_float());
	    //input_b[j*N+k] = int(rand_float());
			input_a[j*N+k] = rand_float();
	    input_b[j*N+k] = rand_float();
	    ref_output[j*N+k] = 0;
	  }
	}
	...
	for (unsigned j = 0; j < N; ++j){
	  for(unsigned k = 0; k < N; ++k){
	    for(unsigned l = 0; l < N; ++l){
	       ref_output[j*N+k] += input_a[j*N+l] * input_b [l*N+k] ;
	    }
	  }
	}
...
// ===========================
     ...
     unsigned char **opencl_program=read_file("matrix_product.cl");
     ...
     if (program == NULL)
	{
     ...
     kernel = clCreateKernel(program, "matrix_product", NULL);
 // Input buffers.
    ...
    checkError(status, "Failed to set argument 1 -> N");

    status = clSetKernelArg(kernel, argi++, sizeof(cl_mem), &input_a_buf);
    checkError(status, "Failed to set argument 2 -> A");

    status = clSetKernelArg(kernel, argi++, sizeof(cl_mem), &input_b_buf);
    checkError(status, "Failed to set argument 3 -> B");

    status = clSetKernelArg(kernel, argi++, sizeof(cl_mem), &output_buf);
    checkError(status, "Failed to set argument 4 -> C");

    const size_t global_work_size[] = {N,N};
		const size_t local_work_size[] = {10,10};
		clock_gettime (CLOCK_REALTIME, &start);
    status = clEnqueueNDRangeKernel(queue, kernel, 2, NULL,
        global_work_size, local_work_size, 2, write_event, &kernel_event);
     checkError(status, "Failed to launch kernel");
    	status=clWaitForEvents(1,&kernel_event);
    checkError(status, "Wait event errorl");
		clock_gettime (CLOCK_REALTIME, &end);
   ...
   diff = (double)(end.tv_sec - start.tv_sec) + (double) (end.tv_nsec - start.tv_nsec) / BILLION;
   printf ("\n \n \n GPU took %.8lf seconds to run.", diff );
	 printf (" This is more than %.1lf times speeder !!! \n \n \n",diffcpu/diff);
	 printf("\n \n Flops/s : %.2f \n \n",N*N*N/diff);


// Verify results.
/*
std::cout<<std::endl<<std::endl<<"GPU RESULT"<<std::endl;
	for(unsigned j = 0; j < N; ++j) {
	  std::cout<<std::endl;
	  for(unsigned k = 0; k < N; ++k){
	    std::cout<<output[j*N+k]<<"           ";
	  }
	}
	std::cout<<std::endl<<std::endl;
*/
...
for(unsigned j = 0; j < N && pass; ++j) {
  for(unsigned k = 0; k  < N && pass; ++k) {
      if(fabsf(output[j*N + k] - ref_output[j*N+k]) > 1.0e-5f) {
        printf("Failed verification @ index %d\nOutput: %f\nReference: %f\n",
            j*N+k, output[j*N+k], ref_output[j*N+k]);
        pass = false;
      }
    }
}
